...
public class UINode extends Group {
	...
	private static final int CHARACTER_SIZE = 7;  // The pixel width of characters in displayed strings
	...
	public UINode(Node node, double xx, double yy) {
		...
		if (node instanceof Endpoint) {
			// If the node itself is the endpoint, add it on the left side, halfway down
			this.endpoints.add(new UIEndpoint((Endpoint) node, this, this.nodeRect.getX(), this.nodeRect.getY() + this.nodeRect.getHeight()/2, true));
		} else {
			int endpointIndex = 0;
			...
			// Find longest endpoint name (to adjust node width to fit endpoint names and the node's name)
			...
			if (this.nodeRect.getWidth() < (longestEndpointNameLeft + longestEndpointNameRight + this.node.getName().length()) * CHARACTER_SIZE + UINODE_WIDTH_PADDING) {
				this.setWidth((longestEndpointNameLeft + longestEndpointNameRight + this.node.getName().length()) * CHARACTER_SIZE + UINODE_WIDTH_PADDING);
			}
			...
			// Create endpoint objects in the appropriate location on the edges of the node
			for (Feature f : node.getFeatures().values()) {
				// put half of the endpoints on the left side
				if (endpointIndex < endpointCount/2) {
					this.endpoints.add(new UIEndpoint((Endpoint) f, this, this.nodeRect.getX(), this.nodeRect.getY() + (ENDPOINT_SIZE * endpointIndex) + ENDPOINT_Y_OFFSET, true));
				} else {
					// put the other half on the right side
					this.endpoints.add(new UIEndpoint((Endpoint) f, this, this.nodeRect.getX() + this.nodeRect.getWidth(), this.nodeRect.getY() + (ENDPOINT_SIZE * (endpointIndex - endpointCount/2)) + ENDPOINT_Y_OFFSET, false));
				}
				endpointIndex++;
			}
		}
